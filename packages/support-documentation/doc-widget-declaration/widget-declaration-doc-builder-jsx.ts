...
export abstract class JsxWidgetDeclarationDocUsageExampleBuilder {
  ...
  protected abstract partImportFramework();

  protected abstract partImport(): ImportDeclaration | undefined;
  ...
  public snippet() {
    const file = makeEsWidgetModuleFile({
      name: "example.tsx",
      path: "tmp/examples",
      imports: [this.partImportFramework(), this.partImport()].filter(
        Boolean
...
export abstract class JsxWidgetDeclarationDocBuilder extends WidgetDeclarationDocBuilder {
  protected readonly sourceuri?: string | undefined;
